...
pgm:
    ...
    { pgmAST = newAST(PROGRAM, $1, 0, NULL, yylineno); return 0; }
    ;
...
final_class_declaration:
    ...
    { $$ = newAST(FINAL_CLASS_DECL, $7, 0, $3, yylineno); appendToChildrenList($$, $9); }
    ;

nonfinal_class_declaration:
    CLASS ID EXTENDS ID LBRACE var_declarations method_decl_check RBRACE
    { $$ = newAST(NONFINAL_CLASS_DECL, $6, 0, $2, yylineno); appendToChildrenList($$, $8); }
    ;
...
type:
    NATTYPE { $$ = newAST(NAT_TYPE, NULL, 0, NULL, yylineno); }
...
main_block:
    MAIN var_expr_block
    { $$ = newAST(MAIN, $2, 0, NULL, yylineno); }
    ;

expr_list:
    expr SEMICOLON { $$ = $1; }
    | expr_list expr SEMICOLON
      { $$ = newAST(EXPR_LIST, $1, 0, NULL, yylineno); appendToChildrenList($$, $2); }
    ;
